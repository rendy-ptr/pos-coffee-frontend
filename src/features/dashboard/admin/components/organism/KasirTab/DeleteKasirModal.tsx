...
const DeleteKasirModal = ({
  ...
  onClose,
  kasirItem,
}: DeleteKasirModalProps) => {
  const { addToast } = useToast();
  const { doDeleteKasir, isPending: isLoadingDelete } = useDeleteKasir();

  const getLoadingMessage = () => {
    if (isLoadingDelete) return 'Menghapus kasir...';
    return 'Loading...';
  };
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md rounded-2xl border border-[#e6d9c9]/50 bg-gradient-to-br from-white to-[#faf9f7] shadow-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Tindakan ini tidak dapat dibatalkan. Data kasir yang dihapus akan
        ...
        <DialogFooter className="mt-6 flex justify-end gap-3">
          ...
          >
            {isLoading ? (
              ...
              <>
                <span className="text-white">Hapus Kasir</span>
              </>
            )}
          </Button>
        </DialogFooter>
...
export default DeleteKasirModal;
