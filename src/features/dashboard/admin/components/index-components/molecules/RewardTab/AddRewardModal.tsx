...
import type { CreateRewardInputPayload } from '../../../../schema/reward.schema';
...
const AddRewardModal = ({ isOpen, onClose }: AddRewardModalProps) => {
  ...
  const { doCreateReward, isPending: isLoading } = useCreateReward();
  const {
    control,
    register,
    handleSubmit,
  ...
  const submitForm = async (data: CreateRewardInputPayload) => {
    try {
      const response = await doCreateReward(data);
