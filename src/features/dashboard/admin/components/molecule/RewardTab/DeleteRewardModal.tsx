...
import { useDeleteReward } from '../../../hooks/reward.hook';
...
const DeleteRewardModal = ({
  ...
  const { doDeleteReward, isPending: isLoadingDelete } = useDeleteReward();

  const getLoadingMessage = () => {
    if (isLoadingDelete) return 'Menghapus Reward...';
    return 'Loading...';
  };
  ...
  const handleDelete = async () => {
    try {
      const response = await doDeleteReward(rewardItem.id);

      if (response.success) {
        addToast(
          response.message || 'Reward berhasil dihapus',
          'success',
          3000
        );
        onClose();
      } else {
        addToast(response.message || 'Gagal menghapus reward', 'error', 3000);
      }
    } catch (err) {
      let message = 'Gagal menghapus reward';
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md rounded-2xl border border-[#e6d9c9]/50 bg-gradient-to-br from-white to-[#faf9f7] shadow-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Tindakan ini tidak dapat dibatalkan. Data reward yang dihapus akan
        ...
        <DialogFooter className="mt-6 flex justify-end gap-3">
          ...
          >
            {isLoading ? (
              ...
              <>
                <span className="text-white">Hapus Reward</span>
              </>
            )}
          </Button>
        </DialogFooter>
